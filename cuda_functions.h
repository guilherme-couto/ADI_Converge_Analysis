...
__global__ void parallelRHSForcing_SSI(real *d_V, real *d_Rv, int N, real t, real delta_t, real delta_x)
{
    ...
    if (index < N*N)
    {
        ...
        real x = j * delta_x;
        real y = i * delta_x;
        forcingTerm = cos(d_pi*x/d_L) * cos(d_pi*y/d_L) * (d_chi*d_Cm*d_pi*cos(d_pi*t) + ((2*d_pi*d_pi*d_sigma)/(d_L*d_L))*sin(d_pi*t) + (d_chi*d_G)*sin(d_pi*t)); 
        
        real actualRHS_V = (forcingTerm/(d_chi*d_Cm)) - (d_G*actualV/d_Cm);
        
        Vtilde = actualV + (0.5 * delta_t) * (((d_sigma/(d_chi*d_Cm)) * diffusion) + actualRHS_V);

        real new_t = t + (0.5 * delta_t);
        forcingTerm = cos(d_pi*x/d_L) * cos(d_pi*y/d_L) * (d_chi*d_Cm*d_pi*cos(d_pi*new_t) + ((2*d_pi*d_pi*d_sigma)/(d_L*d_L))*sin(d_pi*new_t) + (d_chi*d_G)*sin(d_pi*new_t));
        real tildeRHS_V = (forcingTerm/(d_chi*d_Cm)) - (d_G*Vtilde/d_Cm);

        // Update V reaction term
        d_Rv[index] = tildeRHS_V;
    }
}

__global__ void prepareRHS(real *d_V, real *d_RHS, real *d_Rv, int N, real phi)
{
    unsigned int index = blockDim.x * blockIdx.x + threadIdx.x;
    ...
    if (index < N * N)
    {
        ...
        if (i > 0 && i < N-1)
        {
            diffusion += (d_V[(i+1)*N + j] - 2*d_V[index] + d_V[(i-1)*N + j]);
        }
        else if (i == 0)
        {
            diffusion += (2*d_V[(i+1)*N + j] - 2*d_V[index]);
        }
        else if (i == N-1)
        {
            diffusion += (2*d_V[(i-1)*N + j] - 2*d_V[index]);
        ...
        d_RHS[index] = d_V[index] + (phi * diffusion) + (0.5 * d_Rv[index]);
    }
}
