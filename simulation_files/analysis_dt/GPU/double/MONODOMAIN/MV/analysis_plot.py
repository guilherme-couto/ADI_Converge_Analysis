...
for method in methods:
    ...
    try:
        ...
        if len(dt) > 0:
            ...
            plt.loglog(dt, 10**np.polyval(fit, log_dt),
                      label=f'{methods[method]} (slope: {fit[0]:.2f})',
                      **style[method], linestyle='--', linewidth=2)
            
            # Plot the points correspondig to each error x dt
            plt.loglog(dt, error, 'o', **style[method], markersize=8)
            # plt.scatter(dt, error, **style[method], label=f'{methods[method]}', s=100)
...
plt.title('Error and Convergence Analysis', fontsize=20, fontweight='bold')
