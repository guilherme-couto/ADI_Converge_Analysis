...
def read_files(methods, dts, dxs, thetas, analysis_path):
    ...
    for method in methods:
        ...
        for theta in tts:
            ...
            for index in range(len(dts)):
                ...
                error = np.sqrt(float(dx)*float(dx)*aux_sum)
...
def calculate_slope(data, alpha, methods, dts, dxs, thetas, analysis_path):
    analysis_file = open(analysis_path, 'a')
...
def run(alpha):
    ...
    data = read_files(methods, dts, dxs, thetas, analysis_path)

    plot_path = f'./simulation-files/simulation-graphs/convergence-analysis_{alpha}.png'
