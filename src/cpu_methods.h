...
void runSimulation(char *method, real delta_t, real delta_x, real theta)
{
    ...
    if (strcmp(method, "ADI") == 0 || strcmp(method, "SSI-ADI") == 0 || strcmp(method, "SSI-CN") == 0)
    {
    ...
    if (strcmp(method, "SSI-CN") == 0)
    {
        while (timeStepCounter < M)
        {
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                Vtilde[i] = V[i] + diff_term + (0.5f * delta_t * (stim - RHS_V_term));
                ...
                real D = (sigma / (chi));
                real diff_term = D * 0.5f * phi * (im1V - 2 * actualV + ip1V);
                ...
                // Calculate Vtilde
                real actualVtilde = actualV + diff_term + (0.5f * delta_t * (-stim - RHS_V_term));
