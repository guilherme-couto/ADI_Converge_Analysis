...
#ifdef TT2
        real actualX_r1 = d_sV[index].X_r1;
        real actualX_r2 = d_sV[index].X_r2;
        real actualX_s = d_sV[index].X_s;
        real actualm = d_sV[index].m;
        real actualh = d_sV[index].h;
        real actualj = d_sV[index].j;
        real actuald = d_sV[index].d;
        real actualf = d_sV[index].f;
        real actualf2 = d_sV[index].f2;
        real actualfCaSS = d_sV[index].fCaSS;
        real actuals = d_sV[index].s;
        real actualr = d_sV[index].r;
        real actualCa_i = d_sV[index].Ca_i;
        real actualCa_SR = d_sV[index].Ca_SR;
        real actualCa_SS = d_sV[index].Ca_SS;
        real actualR_prime = d_sV[index].R_prime;
        real actualNa_i = d_sV[index].Na_i;
        real actualK_i = d_sV[index].K_i;
...
// Update state variables
        ...
        d_sV[index].W = actualW + delta_t * RHS_Wtilde_term;
#endif // AFHN
        ...
        // Rush-Larsen method - update approximations
        d_sV[index].X_r1 = X_r1_inftilde - (X_r1_inftilde - actualX_r1) * exp(-delta_t / (alpha_X_r1tilde * beta_X_r1tilde));
        d_sV[index].X_r2 = X_r2_inftilde - (X_r2_inftilde - actualX_r2) * exp(-delta_t / (alpha_X_r2tilde * beta_X_r2tilde));
        d_sV[index].X_s = X_s_inftilde - (X_s_inftilde - actualX_s) * exp(-delta_t / (alpha_X_stilde * beta_X_stilde + 80.0f));
        d_sV[index].m = m_inftilde - (m_inftilde - actualm) * exp(-delta_t / (alpha_mtilde * beta_mtilde));
        d_sV[index].h = h_inftilde - (h_inftilde - actualh) * exp(-delta_t * (alpha_htilde + beta_htilde));
        d_sV[index].j = j_inftilde - (j_inftilde - actualj) * exp(-delta_t * (alpha_jtilde + beta_jtilde));
        d_sV[index].d = d_inftilde - (d_inftilde - actuald) * exp(-delta_t / (alpha_dtilde * beta_dtilde + gamma_dtilde));
        d_sV[index].f = f_inftilde - (f_inftilde - actualf) * exp(-delta_t / (alpha_ftilde + beta_ftilde + gamma_ftilde));
        d_sV[index].f2 = f2_inftilde - (f2_inftilde - actualf2) * exp(-delta_t / (alpha_f2tilde + beta_f2tilde + gamma_f2tilde));
        d_sV[index].fCaSS = fCaSS_inftilde - (fCaSS_inftilde - actualfCaSS) * exp(-delta_t / tau_fCaSStilde);
        d_sV[index].s = s_inftilde - (s_inftilde - actuals) * exp(-delta_t / tau_stilde);
        d_sV[index].r = r_inftilde - (r_inftilde - actualr) * exp(-delta_t / tau_rtilde);

        // Explicit method - update approximations
        d_sV[index].R_prime = actualR_prime + (delta_t * RHS_R_primetilde_term);
        d_sV[index].Ca_i = actualCa_i + (delta_t * RHS_Ca_itilde_term);
        d_sV[index].Ca_SR = actualCa_SR + (delta_t * RHS_Ca_SRtilde_term);
        d_sV[index].Ca_SS = actualCa_SS + (delta_t * RHS_Ca_SStilde_term);
        d_sV[index].Na_i = actualNa_i + (delta_t * RHS_Na_itilde_term);
        d_sV[index].K_i = actualK_i + (delta_t * RHS_K_itilde_term);
#endif // TT2
    }
}
