...
bool log_init(FILE *terminal_output, const char *log_file_path, bool use_colors)
{
    terminal_out = terminal_output ? terminal_output : stderr; // Se não passar terminal_output, usa stderr
    colors_enabled = use_colors;  // Configura a variável de cores

    // Tenta abrir o arquivo de log, se fornecido
    if (log_file_path)
    {
        ...
        if (!file_out)
        {
            ...
            return false;  // Retorna falso se não conseguir abrir o arquivo
        }
    }
...
void log_file_content(const char *file_path, const char *description)
{
    ...
    if (config_file == NULL)
    {
        // If the file cannot be opened, log an error message
        if (terminal_out)
        {
            fprintf(terminal_out, "Failed to open file for logging: %s\n", file_path);
        }
        if (file_out)
        {
            fprintf(file_out, "Failed to open file for logging: %s\n", file_path);
        }
        // Log the error
        LOG_ERROR("Failed to open file for logging: %s", file_path);
        return;
    }
    printf("Logging file content: %s\n", file_path);
